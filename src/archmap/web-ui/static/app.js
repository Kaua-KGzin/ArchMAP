...
function setupLiveReload() {
  ...
  source.onmessage = (event) => {
    if (event.data === "reload") {
      init().catch((err) => console.error("[watch] Failed to refresh:", err));
    }
  };
  source.onerror = () => {
    source.close();
    liveReloadInitialized = false;
...
function renderImpactAnalysis(impact) {
  const riskKey = `impact.risk.${impact.risk}`;
  const riskClass = `insights-pill-${impact.risk}`;

  let html = `
    <div class="space-y-3 pt-4 border-t border-outline-variant/10">
      <div class="flex justify-between items-center">
        <h3 class="text-[10px] font-bold text-on-surface-variant uppercase">${escapeHtml(t("impact.title"))}</h3>
        <span class="history-pill ${riskClass} text-[9px]">${escapeHtml(t(riskKey))}</span>
      </div>

      <p class="text-[11px] font-medium text-on-surface-variant/80">
        ${escapeHtml(t("impact.count", { count: impact.impactCount }))}
      </p>

      <ul class="text-[11px] space-y-1 max-h-32 overflow-y-auto custom-scrollbar pr-2">
        ${impact.impactedFiles.map(f => `<li class="text-on-surface/70 truncate">• ${escapeHtml(f)}</li>`).join("")}
      </ul>
    </div>
  `;
  return html;
...
function renderTimelineItem(snapshot) {
  const li = document.createElement("li");
  li.className = "relative pl-4 space-y-1";

  if (snapshot.regression) {
    li.innerHTML = `
      <div class="absolute -left-1.5 top-1.5 w-3 h-3 rounded-full bg-secondary shadow-[0_0_8px_var(--secondary-glow)]"></div>
      <div class="p-3 bg-secondary/10 border border-secondary/20 rounded-xl space-y-2">
        <div class="flex justify-between items-start">
          <div>
            <p class="text-[10px] font-bold text-secondary uppercase tracking-tighter">${escapeHtml(t("history.regression"))}</p>
            <p class="text-[11px] font-bold text-on-surface">${escapeHtml(snapshot.shortCommit)} • ${escapeHtml(snapshot.author)}</p>
          </div>
          <span class="history-pill history-pill-warn text-[9px]">${snapshot.health}</span>
        </div>
        <p class="text-[11px] text-on-surface-variant leading-tight">${escapeHtml(snapshot.subject)}</p>
      </div>
    `;
  } else {
    li.innerHTML = `
      <div class="absolute -left-1.5 top-1.5 w-3 h-3 rounded-full bg-surface-bright border-2 border-outline"></div>
      <div class="flex justify-between items-start group">
        <div>
          <p class="text-[11px] font-medium text-on-surface group-hover:text-primary transition-colors">${escapeHtml(snapshot.shortCommit)} • ${escapeHtml(snapshot.author)}</p>
          <p class="text-[10px] text-on-surface-variant/60 truncate w-40">${escapeHtml(snapshot.subject)}</p>
        </div>
        <span class="history-pill text-[9px]">${snapshot.health}</span>
      </div>
    `;
  }
  return li;
...
function renderInsights(report) {
  ...
  if (!insights || !elements.insightsPanel) {
  ...
  const statusKey = `insights.status.${insights.status}`;
  elements.insightsStatus.textContent = t(statusKey);
  elements.insightsStatus.className = `history-pill insights-pill-${insights.status}`;

  let html = `<p class="text-sm font-bold text-on-surface">${escapeHtml(insights.message)}</p>`;

  if (insights.problems && insights.problems.length > 0) {
    html += `
      <div class="space-y-2 pt-2">
        <p class="text-[10px] font-bold text-on-surface-variant/50 uppercase">${escapeHtml(t("insights.problems"))}</p>
        <ul class="text-[11px] space-y-2">
          ${insights.problems.map(p => `<li class="flex gap-2"><span class="text-secondary">•</span> <span>${escapeHtml(p)}</span></li>`).join("")}
        </ul>
      </div>
    `;
  }
  ...
  if (insights.actions && insights.actions.length > 0) {
    html += `
      <div class="space-y-2 pt-2 border-t border-outline-variant/10">
        <p class="text-[10px] font-bold text-on-surface-variant/50 uppercase">${escapeHtml(t("insights.actions"))}</p>
        <ul class="text-[11px] space-y-2">
          ${insights.actions.map(a => `<li class="flex gap-2"><span class="material-symbols-outlined text-xs text-primary">lightbulb</span> <span>${escapeHtml(a)}</span></li>`).join("")}
        </ul>
      </div>
    `;
  }
...
function renderHistory(history) {
  ...
  for (const snapshot of snapshots.slice().reverse()) {
    elements.historyTimeline.appendChild(renderTimelineItem(snapshot));
  }
...
function bindControls() {
  ...
  elements.openFileBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      let detailsHtml = "";
      if (node.impact && node.impact.impactCount > 0) {
        detailsHtml += renderImpactAnalysis(node.impact);
      }

      elements.selectionInfo.innerHTML = detailsHtml + `
